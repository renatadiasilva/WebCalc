...
public class Calculator implements Serializable {
	...
	private ExpressionC expression;
	...
	public ExpressionC getExpression() {
		return expression;
	}
	...
	public String result() {
		...
		try {
			Expression e = new ExpressionBuilder(expression.getExp())
					.function(pi)
					.function(sind)
					.function(cosd)
					.function(tand)
	...
	public void addToExpression(boolean delL, String add) {
		if (clean) expression.setExp(add);
		else if (delL) {
				int l = expression.getExp().length();
	...
	public void key(String s) {
		...
		if (expression.getExp().equals("0")) clean = !op;
	...
	public int countsOperation(String op) {
	    ...
	    Matcher m = p.matcher(expression.getExp());
